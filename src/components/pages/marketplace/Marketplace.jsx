...
const Marketplace = () => {
	...
	return (
		...
		>
			...
			<div className='container mx-auto max-w-screen-lg m-0-auto'>
				<MarketplaceGlobal />
				<div className='px-5 md:px-0 flex flex-col items-center justify-center bg-blue-700 rounded-lg text-white mt-24'>
					<h1 className=' mt-24 font-bold text-2xl md:text-4xl'>
					...
					<div className='flex flex-wrap gap-3 mt-9 mb-24'>
						{data.map((data, index) => (
							...
							>
								<h1 className=' text-1xl md:text-4xl font-bold'>{data.title}</h1>
								<span className=' inline-block w-12 border-t-4 bg-indigo-600 mt-1'></span>
								<p className=' md:text-base text-slate-400 mt-4'>{data.text}</p>
							</div>
						))}
					</div>
				</div>
			</div>
			<div className='mt-24 py-12 px-3 md:px-0 bg-gray-100'>
				<div className='container mx-auto max-w-screen-lg m-0-auto'>
